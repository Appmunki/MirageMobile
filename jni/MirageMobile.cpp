...
template<class T, class U>
  bool
  compare(const pair<T, U> &a, const pair<T, U> &b)
  {
...
//    LOG(
//        "drawhomography line 0: (%d,%d) to (%d,%d)", scene_corners[0].x, scene_corners[0].y, scene_corners[1].x, scene_corners[1].y);
//    LOG("drawhomography points 1: (%d,%d)", 1, 2);
//    LOG(
//        "drawhomography line 1: (%d,%d) to (%d,%d)", scene_corners[1].x, scene_corners[1].y, scene_corners[2].x, scene_corners[2].y);
//    LOG("drawhomography points 3: (%d,%d)", 2, 3);
//    LOG(
//        "drawhomography line 2: (%d,%d) to (%d,%d)", scene_corners[2].x, scene_corners[2].y, scene_corners[3].x, scene_corners[3].y);
//    LOG("drawhomography points 3: (%d,%d)", 3, 0);
//    LOG(
//        "drawhomography line 3: (%d,%d) to (%d,%d)", scene_corners[3].x, scene_corners[3].y, scene_corners[0].x, scene_corners[0].y);
...
//
  ...
  JNIEXPORT void JNICALL
  Java_com_appmunki_miragemobile_ar_Matcher_convertFrame(JNIEnv* env,
      jobject obj, jint width, jint height, jbyteArray yuv, jintArray rgba)
  {
    //Conversion of frame
    jbyte* _yuv = env->GetByteArrayElements(yuv, 0);
...
//      }
  ...
  JNIEXPORT void JNICALL
  Java_com_appmunki_miragemobile_ar_Matcher_FindFeatures(JNIEnv* env,
      jobject thiz, jint width, jint height, jbyteArray yuv, jintArray rgba,
      jintArray gray)
  {
    jbyte* _yuv = env->GetByteArrayElements(yuv, 0);
    jint* _rgba = env->GetIntArrayElements(rgba, 0);
    ...
    for (size_t i = 0; i < size; i++)
      {
        ...
        circle(mrgba, Point(v[i].pt.x, v[i].pt.y), 5, Scalar(255, 191, 0, 255),
            -1);

        //place circles on YUV Image
        circle(myuv, Point(v[i].pt.x, v[i].pt.y), 5, Scalar(255, 191, 0, 255),
            -1);

        //place keypoints on Gray Image
        circle(mgray, Point(v[i].pt.x, v[i].pt.y), 5, Scalar(255, 191, 0, 255),
            -1);
