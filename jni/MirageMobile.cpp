...
extern "C" {
  ...
  void
  buildProjectionMatrix(const CameraCalibration& calibration, int screen_width, int screen_height, Matrix44& projectionMatrix);
  ...
  */
   ...
   */
  void
  readKeyAndDesc(vector<KeyPoint> &trainKeys, Mat &trainDes, float *mdata, int &count) {
    // doc du lieu
   ...
   */
  inline void
  readDatabase(float *mdata, int &count) {
  ...
  inline bool
  refineMatchesWithHomography(int it, float &confidence, const std::vector<cv::KeyPoint>& queryKeypoints,
   ...
   */
  inline void
  match(const Mat& m_grayImg, const vector<KeyPoint> &trainKeys, const Mat &trainDes, vector<pair<float, int> > &result) {
    int runs = 0;
