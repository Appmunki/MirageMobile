...
PatternDetector::PatternDetector(cv::Ptr<cv::FeatureDetector> detector, cv::Ptr<cv::DescriptorExtractor> extractor,
		cv::Ptr<cv::DescriptorMatcher> matcher, bool ratioTest) :
		m_detector(detector), m_extractor(extractor), m_matcher(matcher), enableRatioTest(ratioTest), enableHomographyRefinement(false), homographyReprojectionThreshold(
				3) {
}
...
void PatternDetector::startTimer() {
	gettimeofday(&t1, NULL);
}

void PatternDetector::stopTimer(char* text) {
...
bool PatternDetector::findPattern(const cv::Mat& image, PatternTrackingInfo& info) {
	...
	stopTimer("getMatches");
	startTimer();

	cv::Mat dest = image.clone();

cv::drawKeypoints(image,m_queryKeypoints,dest,cv::Scalar(255,0,0),1);
	cv::imwrite("/mnt/sdcard/MirageTest/features.jpg",dest);
	// Find homography transformation and detect good matches
	bool homographyFound = refineMatchesWithHomography(m_queryKeypoints, m_pattern.keypoints, homographyReprojectionThreshold, m_matches,
			m_roughHomography);
	stopTimer("Homography");
	startTimer();
	if (homographyFound) {
		...
		if (enableHomographyRefinement) {
			...
			homographyFound = refineMatchesWithHomography(warpedKeypoints, m_pattern.keypoints, homographyReprojectionThreshold, refinedMatches,
					m_refinedHomography);
...
bool PatternDetector::refineMatchesWithHomography(const std::vector<cv::KeyPoint>& queryKeypoints, const std::vector<cv::KeyPoint>& trainKeypoints,
	...
	LOG("MATCHES SIZE %d",matches.size());
